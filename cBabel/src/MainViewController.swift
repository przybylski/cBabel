...
extension NSMenuItem {
  convenience init(title: String, keyEquivalent: String = "", actionClosure: @escaping () -> Void) {
    let closureTarget = ClosureMenuItem(action: actionClosure)
    
    self.init(title: title, action: #selector(closureTarget.performAction), keyEquivalent: keyEquivalent)
    self.target = closureTarget
    
    objc_setAssociatedObject(self, "closureTarget", closureTarget, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
  }
}
    ...
    do {
      ...
      if let messagePath = messageUrl?.path {
        ...
        if let data = data as? Data {
          let jsonOpt = try JSONSerialization.jsonObject(
            with: data,
    ...
    for (key, value) in availableLanguages {
      let action = {
        ...
        if let path = value?.path, let parsed = self.parseLocale(path: path) {
          ...
          self.translatedTextPath = value?.appendingPathComponent("messages.json")
  ...
  @IBAction func openChooserDialog(sender : NSObject) {
    ...
    panel.beginSheetModal(for: self.view.window!, completionHandler: {(result) -> Void in
      if result == NSApplication.ModalResponse.OK {
        ...
        if let localeDirPath = localeDir?.path, let languageCode = languageCode {
          self.readOtheLocales(localeDir: localeDirPath, mainLocale: languageCode)
        }
        
        if let mainLocale = dirUrl?.path, let content = self.parseLocale(path: mainLocale) {
